<template>
  <a-layout>
    <nav class="bg-stone-100 px-4 py-2 flex items-center justify-between border">
      ...
      <div class="flex" v-if="accountStore.isAccountEmpty">
        ...
        <router-link :to="{ name: 'registration' }">
          <button class="bg-sky-200 hover:bg-sky-300 transition-colors text-sm font-bold px-6 py-2 rounded-lg pt-1">Регистрация</button>
        </router-link>
      </div>
    </nav>
    <a-layout>
      ...
      <a-layout style="padding: 0 24px 24px">
        ...
        >
          ...
          <AlbomItem v-if="navbarItems === NavbarItems.ALBOMITEM" :music="music" @show-audio="showModal"/>

          <Table v-if="navbarItems === NavbarItems.TRICS">
            <TrackList v-for="data in searchResponse"
            :key="data.id"
            :data="data"
            @show-audio="showModal"
            />
          </Table>
          <AlbumList v-if="navbarItems === NavbarItems.ALBUMS" v-for="data in searchResponse" :key="data.id" :data="data"
          @send-music="setMusic"/>
        </a-layout-content>
      </a-layout>
    </a-layout>
...
// Search
const search = ref<string>("Superman");
